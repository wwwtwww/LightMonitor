  ...
  <div>
    ...
    >
      ...
      <template #extra="{ getLatest }">
        ...
        <el-table-column label="Active Sessions" width="140" align="right" header-align="right" show-overflow-tooltip>
          <template #default="scope"><span class="lm-num">{{ formatNumber(parseExtra(getLatest(scope.row)).activeSessions ?? 0) }}</span></template>
        </el-table-column>
      </template>
    </TargetsTable>
...
const oracleRows = computed(() => (props.rows || []).filter(r => (r.type || '').toLowerCase() === 'oracle'))

const treeRows = computed(() => {
  const tree = buildTargetTree(oracleRows.value || [])
